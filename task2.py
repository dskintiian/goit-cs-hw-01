...
class Token:
    def __init__(self, type, value, priority = None):
        self.type = type
        self.value = value
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            match self.current_char:
                ...
                case "*":
                    self.advance()
                    return Token(TokenType.MUL, "*", 1)
                case "/":
                    self.advance()
                    return Token(TokenType.DIV, "/", 1)
                case "(":
                    self.advance()
    ...
    def generate_postfix(self):
        ...
        while self.current_token.type != TokenType.EOF:
            if self.current_token.type == TokenType.INTEGER:
                self.postfix_stack.append(self.current_token)
            elif self.current_token.type == TokenType.LPAREN:
                buffer.append(self.current_token)
            elif self.current_token.type == TokenType.RPAREN:
                while buffer and buffer[-1].type != TokenType.LPAREN:
                    self.postfix_stack.append(buffer.pop())
                buffer.pop()
            else:
                while buffer and buffer[-1].type != TokenType.LPAREN and self.current_token.priority <= buffer[-1].priority:
                    self.postfix_stack.append(buffer.pop())
                buffer.append(self.current_token)
    ...
    def term(self):
        ...
        while self.postfix_stack:
            ...
            if token.type == TokenType.INTEGER:
                tree_stack.append(Num(token))
            else:
                right = tree_stack.pop()
                left = tree_stack.pop()
                tree_stack.append(BinOp(left=left, op=token, right=right))
